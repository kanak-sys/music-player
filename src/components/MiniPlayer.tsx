...
export const MiniPlayer: React.FC = () => {
  ...
  const { togglePlayPause, skipNext } = useAudioPlayer();
  ...
  return (
    ...
    >
      <View style={styles.inner}>
        <Image source={{ uri: getBestImageUrl(currentSong) }} style={styles.image} />
        <View style={styles.info}>
          <Text style={styles.name} numberOfLines={1}>{currentSong.name}</Text>
          <Text style={styles.artist} numberOfLines={1}>{getSongArtists(currentSong)}</Text>
        </View>
...
const styles = StyleSheet.create({
  container: {
    ...
    backgroundColor: Colors.miniPlayer, borderRadius: 16,
    shadowColor: '#000', shadowOffset: { width: 0, height: -4 },
    shadowOpacity: 0.4, shadowRadius: 12, elevation: 12,
    borderWidth: 1, borderColor: Colors.border,
  },
  inner: { flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 12 },
  image: { width: 44, height: 44, borderRadius: 8, backgroundColor: Colors.surface },
  info: { flex: 1, marginLeft: 12, marginRight: 8 },
  name: { color: Colors.text, fontSize: Fonts.sizes.md, fontWeight: '600', marginBottom: 2 },
